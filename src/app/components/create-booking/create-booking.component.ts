...
//   details: any[] = [];  // Thêm quantity default 1
//   extras: any[] = [];  // List extras
//   allExtras: any[] = [];  // Danh sách extras load từ API cho lookup
//   customers: any[] = [];
//   searchQuery: string = '';
//   showAddCustomer: boolean = false;
//   showSearchCustomer: boolean = false;
...
//     private bookingStatusService: BookingStatusService,
//     private paymentMethodService: PaymentMethodService,
//     private extraService: ExtraService,  // Thêm
//   ) { }
...
//     this.calculateTotal();
//   }
//   // Hiển thị tên phụ thu cho cột extraId
//   getExtraName = (row: any) => {
//     if (!row || !row.extraId) return "";
//     const extra = this.allExtras.find(x => x.id === row.extraId);
...
//         this.allExtras = response.data;
//         this.extraPriceMap = {};
//         this.allExtras.forEach(x => {
//           this.extraPriceMap[x.id] = x.price;
//         });
//       } else {
//         this.toastr.error(response.name, 'Lỗi');
//       }
...
//   calculateExtraSubtotal(extra: any): number {
//     const days = (new Date(this.booking.checkOut).getTime() - new Date(this.booking.checkIn).getTime()) / (1000 * 3600 * 24);
//     switch (extra.unit) {  // Giả định extra có unit từ API (per_night, per_stay, per_person, fixed)
//       case 'per_night':
//         return extra.priceAtBooking * days * extra.quantity;
//       case 'per_stay':
//         return extra.priceAtBooking * extra.quantity;
//       case 'per_person':
//         return extra.priceAtBooking * extra.quantity;  // Giả định num people = quantity
//       case 'fixed':
//         return extra.priceAtBooking;
//       default:
//         return 0;
//     }
//   }
...
//   // Cập nhật giá và subtotal
//   updateExtraPricing(row: any) {
//     const extra = this.allExtras.find(x => x.id === row.extraId);
//     if (extra) {
//       row.priceAtBooking = extra.price;
//       row.subtotal = row.quantity * extra.price;
//     }
//   }
...
//   onExtraSelectionChanged(event: any, rowData: any) {
//     if (event.value) {
//       const selectedExtra = this.allExtras.find(ex => ex.id === event.value);
//       if (selectedExtra) {
//         rowData.priceAtBooking = selectedExtra.price;
//         rowData.unit = selectedExtra.unitId;  // Giả định unit từ backend là unitId, adjust nếu cần
//         rowData.subtotal = this.calculateExtraSubtotal(rowData);
//         this.calculateTotal();
//       }
//     }
